	...
	function SaveSchedule(){
		...
		new Ajax.Request(WEB_ROOT+'/ajax/schedule-student.php', 
		{
			method:'post',
	...
	function confirm_schedule(){
		...
		if(!confirm('Esta seguro de confirmar su inscripción?')){		
			return;
		}
		...
		{
			method:'post',
			parameters: {type: "confirmSchedule"},
		onSuccess: function(transport){
				var response = transport.responseText || "no response text";
				...
				{
					//ShowStatusPopUp(splitResponse[1])
					location.href = WEB_ROOT + 'schedule-student';
				}
				grayOut(false);	
			},
		onFailure: function(){ alert('Something went wrong...') }
	...
	function confirm_scheduleV1(idUser, idSchGpo){
		...
		{
			method:'post',
			parameters: {type: "confirmScheduleV1", userId:idUser, schGpoId:idSchGpo},
		onSuccess: function(transport){
				var response = transport.responseText || "no response text";
	...
	function PrintSchedule(userId){
		...
		location.href = WEB_ROOT+'schedule-student/imp/'+userId;
