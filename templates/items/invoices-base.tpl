{foreach from=$invoices item=subject}
  ...
  {*}?invoiceId={$subject.invoiceId}{*}
                        ...
                        {if $subject.payment>0} <a href="http://www.iapchiapasenlinea.mx/invoicesrep.php?invoiceId={$subject.invoiceId}" target="_blank" title="Imprimir pagos" > <img src="{$WEB_ROOT}/images/icons/16/pdf.gif" class="spanPay" id="d-{$subject.invoiceId}" name="d-{$subject.name}"  /></a>        {/if}
        {else}
	        {if $subject.debt > 0}    
