...
*/
		...
		if( $_SESSION['User']['perfil']<>'Administrador')	{
			if($datTop["tipo"] == "dudas"){
				...
				$sendmail->PrepareAttachment("Asesoria Academica",$this->reply, "", "", EMAIL_USERNAME, "Administrador", $attachment, $fileName);
			}
			...
			if($datTop["tipo"] == "asesoria"){
				...
				$this->Util()->DB()->setQuery($sql);
				$infoDu = $this->Util()->DB()->GetRow();
				$sendmail->PrepareAttachment("Asesoria Academica",$concatMsj.$this->reply, "", "", EMAIL_USERNAME, "Administrador", $attachment, $fileName);
			}
		}
